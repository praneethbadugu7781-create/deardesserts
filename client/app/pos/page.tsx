...
export default function PosBillingPage() {
  ...
  const loadDisabledItems = () => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('dd_disabled_items');
      if (stored) {
        try {
          setDisabledItemIds(JSON.parse(stored));
        } catch (e) {}
      } else {
        setDisabledItemIds([]);
      }
...
const REAL_CATEGORIES: Category[] = [
  ...
  const addToCart = (item: MenuItem) => {
    if (!item.isAvailable || disabledItemIds.includes(item.id)) {
      alert(`⚠️ "${item.name}" is currently OUT OF STOCK and cannot be sold!`);
      return;
    }
    setCart((prev) => {
        ...
        <div className={`lg:col-span-7 space-y-6 ${mobileTab === 'MENU' ? 'block' : 'hidden lg:block'}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
            {filteredMenuItems.map((item) => {
              const isOut = !item.isAvailable || disabledItemIds.includes(item.id);
              return (
                <div
                  key={item.id}
                  onClick={() => addToCart(item)}
                  className={`bg-white/90 backdrop-blur-md rounded-2xl p-4 border shadow-sm transition-all flex flex-col justify-between group ${
                    isOut
                      ? 'opacity-60 border-red-300 bg-red-50/40 cursor-not-allowed'
                      : 'border-cream-300/80 hover:border-gold-500/80 hover:bg-gold-50/30 cursor-pointer hover:shadow-md hover:-translate-y-0.5'
                  }`}
                >
                  <div>
                    <div className="flex items-center justify-between gap-1 mb-1.5">
                      ...
                      {isOut && (
                        <span className="text-[9px] font-black uppercase tracking-wider text-red-600 bg-red-100 px-2 py-0.5 rounded-full border border-red-200">
                          OUT OF STOCK
                        </span>
                      )}
                    </div>
